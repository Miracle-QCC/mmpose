...
def parse_args():
    ...
    parser.add_argument('--onnx_file', help='ONNX file path', default='/home/qcj/workcode/mmpose/onnx/rtmpose_t_68.onnx')
    parser.add_argument('--image_file', help='Input image file path',default='/home/qcj/workcode/mmpose/tools/face_11_12_frm_2014_qua_0.841.png')
    parser.add_argument(
        '--device', help='device type for inference', default='cpu')
    parser.add_argument(
        '--save-path',
        help='path to save the output image',
        default='/home/qcj/workcode/mmpose/tools/1.jpg')
    ...
    for kpts, score in zip(keypoints, scores):
        for kpt, color in zip(kpts, point_color):
            cv2.circle(img, tuple(kpt.astype(np.int32)), 1, palette[color], 1,
                       cv2.LINE_AA)
            # cv2.putText(img, str(idx), tuple(kpt.astype(np.int32)), 1, 1, (0, 255, 0), 1)
            idx += 1
